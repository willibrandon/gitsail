...
Every rendered frame must completely replace the cells it owns. Full-screen sessions use cell-positioned updates on every platform so content at the terminal's final column never wraps into the next row. Long-lived editor state replaces its document contents in place so the editor invalidates every changed cell, including blank tails that erase longer text and colored diff spans from the previous preview. Real history and diff tests scroll vertically and horizontally in both directions, replace a long preview with a shorter preview, and verify that no text, styling, or shifted right edge from the previous viewport remains.
