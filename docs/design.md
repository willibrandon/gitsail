...
Every rendered frame must completely replace the cells it owns. Full-screen sessions use cell-positioned updates on every platform so content at the terminal's final column never wraps into the next row. Long-lived editor state replaces its document contents in place so the editor invalidates every changed cell, including blank tails that erase longer text and colored diff spans from the previous preview. Selecting a different history entry starts its preview at the first row and first column; refreshing the same selected commit retains its cursor and viewport. Real history and diff tests scroll vertically and horizontally in both directions, change entries, replace a long preview with a shorter preview, and verify that the new preview starts at the origin with no text, styling, or shifted right edge from the previous viewport.
