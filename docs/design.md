...
History-list focus changes update the selected row and commit details immediately. They never await `git show`. Preview requests use a short settle delay, cancel and reap superseded child processes, and apply output only when both the request generation and focused object ID still match. A fast mouse-wheel or key-repeat burst therefore loads the final focused commit instead of serially loading every intermediate row.
