...
Every rendered frame must completely replace the cells it owns. Full-screen sessions use cell-positioned updates on every platform and disable terminal automatic wrapping before the first frame, so drawing the final column cannot wrap into the next row or shift later writes. Every clean Windows history frame disables automatic wrapping again before clearing and repainting. The output queue holds at most one pending item and applies backpressure instead of letting incremental frames accumulate behind a slower terminal. Long-lived editor state replaces its document contents in place so the editor invalidates every changed cell, including blank tails that erase longer text and colored diff spans from the previous preview. Selecting a different history entry starts its preview at the first row and first column; refreshing the same selected commit retains its cursor and viewport. On Windows, changing a history entry, finishing its delayed preview load, or moving its preview viewport by keyboard, wheel, click, or scrollbar drag schedules the clean repaint on the application render thread. GitSail applies the clear to its terminal model and folds the physical clear into the beginning of the next synchronized full frame, so no partial frame or competing output can leave text or color from an older preview on screen. If a history update arrives while a frame is already being built, GitSail requests another frame until the newest selection and preview have been built together. Real history and diff tests scroll vertically and horizontally in both directions, change entries during frame construction, replace a long preview with a shorter preview, and verify that the new preview starts at the origin with no text, styling, or shifted right edge from the previous viewport.
