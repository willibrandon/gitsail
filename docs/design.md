...
- Helper modes are absent from public help and completion. Askpass and yes/no helpers are selected by authenticated private environment variables before public command dispatch. Sequence-editor and editor bridges use hidden System.CommandLine commands plus the authenticated private environment described in §11.7.
...
The .NET 10 Unix implementation keeps the exact-byte ABI in `UnixNative` and uses the Native AOT-linked `System.Native` fork/`execve` portability entry point. GitSail constructs and owns NUL-terminated `argv` and `envp` blocks, passes the canonical raw-byte working directory, receives close-on-exec redirected pipes, and reaps the exact returned PID. An explicit terminal-attached variant inherits standard input, output, and error only after GitSail has stopped and restored its own TUI; interactive rebase uses that variant so Git's sequence editor, commit editor, hooks, and prompts receive the real terminal. This avoids managed code after `fork`, adds no helper executable or native package asset, and uses the same platform implementation already carried by the self-contained runtime. The ABI is isolated to one file and guarded by framework-dependent, Linux, macOS, and Native AOT execution tests; adopting another target-framework major requires an explicit ABI review rather than runtime-major roll-forward. Windows uses only `ProcessStartInfo.ArgumentList`, a cleared explicit environment, and UTF-16 paths.
...
The header never relies on terminal clipping. At 180 columns and wider, one row shows the workflow, bounded branch/tracking text, repository label, complete ordinary Git version, and repository action buttons. From 120 through 179 columns it removes secondary fields before shortening values. Below 120 columns it gives version/status and repository actions separate rows so Commands, Branches, Remotes, and Stashes remain readable and clickable. Unusually long branch names, repository labels, and vendor version suffixes use a visible ellipsis; standard Windows forms such as `2.51.1.windows.1` retain their original separator and remain complete.

File lists are virtualized, filterable by path/status, and preserve selection by raw path across generations. They support keyboard range selection, Ctrl/Shift mouse selection, status badges that never rely on color alone, rename old/new names, conflict stages, submodule summaries, and an escaped-path detail view. Mouse-wheel movement advances one row and stops at the first or last item instead of wrapping to the opposite end. A configured untracked display cap affects rendering only, not correctness or stage-all behavior.
...
The commit message editor uses lifted `EditorState`, preserves drafts across reconciliation and failed hooks, supports undo/redo, optional hard wrap, spelling, signoff, templates, author override, and amend. Live spelling waits for 300 ms of editing inactivity, cancels the superseded check, and accepts results only for the exact current document version. Possible misspellings receive theme-aware curly underlines; Shift+F7 reviews the current or next issue, and right-clicking a marked word opens its bounded replacement list. Applying a suggestion verifies the original range still contains the checked word before replacing it. Spelling state never changes commit eligibility. Initial content has one exact precedence order: a present `GITGUI_EDITMSG`, `GITGUI_MSG`, or `GITGUI_BCK` saved commit message file; a pending merge message; a pending squash message; the exact selected HEAD body for an amend session; the effective `commit.template`; then an empty document. A present empty saved message file is intentional and still wins. Lower-precedence sources never overwrite a higher-precedence message.
...
Line gutters show repository file coordinates, never the row number of the presentation document. Unified view has separate old and new columns: context rows show both, deletions show only the old number, additions show only the new number, and metadata and hunk headers remain blank. Side-by-side view shows only old numbers on the left and new numbers on the right while retaining blank aligned rows.
...
Remote prune shows Git's complete dry-run output and captures it again immediately before execution. The guard combines standard output and standard error and compares the exact state-line multiset, including duplicates, while ignoring stream placement, line order, host newlines, and Git's presentation-only `Pruning` and `URL` headings. The complete remote catalog is revalidated separately, so a changed remote, prunable target, warning, or duplicate count rejects the confirmation without treating Windows path formatting as repository state.
...
History is native to the TUI rather than delegated to gitk. It obtains structured commit records and parent OIDs from Git, builds a bounded lane graph, virtualizes long histories, shows refs and signatures, filters by path/author/text, and opens commit diffs. Cherry-pick and Revert commit are separate, clearly labeled actions so Revert commit cannot be confused with discarding worktree changes.

Each action is cancel-first and shows the complete selected commit OID, control-safe subject, current branch or detached HEAD, and the exact effect before Git runs. A merge commit requires an explicit one-based mainline-parent choice and shows that choice again in the final confirmation. Escape and a pointer click outside close each selector or confirmation without changing the repository. The prepared action validates `GIT_COMMITTER_IDENT`, captures HEAD, symbolic attachment, the complete index, and the action-relevant worktree; the service rechecks identity and all repository state immediately before invoking `git cherry-pick --no-edit` or `git revert --no-edit` with the exact confirmed OID and optional mainline parent.

Git-owned `CHERRY_PICK_HEAD` and `REVERT_HEAD` queries classify a stopped operation without parsing localized error text. History detects an already-stopped operation at startup and replaces its ordinary mutation actions with Continue, Skip, and Abort. Continue runs directly after the user resolves and stages files. Skip and Abort are cancel-first confirmations bound to the exact displayed operation and commit. Every completion, stop, skip, or abort reloads history and operation state; Git retains ownership of commits, hooks, conflicts, messages, rollback, and state cleanup.

History-list focus changes update the selected row and commit details immediately. They never await `git show`. Up, Down, J, K, and mouse-wheel movement stop at the first and last visible commits instead of rolling over to the opposite end. Movement at a boundary does not invalidate the screen or request the same preview again. Preview requests use a short settle delay, cancel and reap superseded child processes, and apply output only when both the request generation and focused object ID still match. A fast mouse-wheel or key-repeat burst therefore loads the final focused commit instead of serially loading every intermediate row.

`git-tui rebase [--onto revision] [upstream]` is owned by System.CommandLine from tokenization through typed `RebaseOptions`; neither operand is reparsed or discarded. An omitted upstream resolves through the current branch's `@{upstream}`. An omitted new base uses the resolved upstream. The planning view also exposes both fields so a user can correct or supply them without restarting. `RevisionResolver` peels each candidate to an exact commit OID with option interpretation disabled. The confirmation shows the complete current HEAD, upstream and new-base OIDs, symbolic branch or detached state, and exact commit count. Preparation validates `GIT_COMMITTER_IDENT` and captures HEAD, attachment, the complete index, and action-relevant worktree identity. Execution reacquires the rebase mutation lease, repeats those captures, proves the exact commit range is unchanged, and passes only the confirmed OIDs to Git.

Interactive rebase uses Git itself. After final confirmation, the parent TUI closes and restores the terminal before a terminal-attached `git rebase --interactive --onto=<exact-oid> <exact-upstream-oid>` starts. Git therefore retains its normal configuration, hooks, signing, conflict, editor, ref, autostash, and rollback behavior and any configured commit editor has a real terminal. The parent does not run a nested terminal application and does not inspect localized progress prose.

`GIT_SEQUENCE_EDITOR` names the current executable's hidden `sequence-editor` System.CommandLine command. Native tool payloads invoke their exact executable; framework-dependent development runs safely quote both the `dotnet` host and exact managed assembly. The command ends with a quoted `--`, so the todo path Git appends is captured from native `argv` and forwarded as an exact UTF-16 Windows path or Unix byte path after System.CommandLine has selected the helper command. Before launching Git, GitSail resolves the sole allowed todo path through `git rev-parse --path-format=absolute --git-path rebase-merge/git-rebase-todo` and creates a random 256-bit, five-minute, single-use request in a no-follow user temp location. The request binds the exact native todo path, expiration, and path kind under HMAC-SHA-256. The request path and secret travel only in `GITSAIL_HELPER_*` variables. The helper authenticates with fixed-time comparison, consumes the request exactly once before opening the todo, independently resolves the repository allowlist path, and requires all three paths to match. Git invocations without a GitSail parent may use the same minimal editor only after its supplied path independently matches that repository's allowed todo path.

The todo parser is bounded, rejects NUL and unknown commands, retains every original byte and line ending, and recognizes Git's full current command set and short aliases: pick, reword, edit, squash, fixup including `-C`/`-c`, drop, exec, break, label, reset, merge, update-ref, and noop. Changing a commit action replaces only its ASCII command token and, when leaving `fixup -C` or `fixup -c`, removes that fixup-only option while retaining the exact target and subject bytes. Reordering swaps command records without moving comments or changing line-ending positions. The editor validates that squash/fixup has a prior result, provides explicit action and move controls, and returns the file through a no-follow atomic replacement. Existing or newly added exec lines prevent Save until the user accepts a confirmation explaining that Git will run the exact command through a shell. Adding exec requires a separate “Add and trust” confirmation; removing it does not execute anything.

The todo editor and parent rebase view are complete at 60×18, keep the complete Git version visible, support pointer focus, selection, scrolling, buttons, window resizing, and click-away dismissal, and make Escape cancel every popup. The parent detects active merge-backend and apply-backend rebase markers without treating a stale `REBASE_HEAD` as an active transaction. A stopped rebase exposes Resolve files, Continue, Skip, Edit Todo only when the interactive todo exists, and Abort. Resolve files opens the ordinary diff, built-in conflict editor, and staging workspace in a rebase-safe mode where every ordinary commit entry point is removed; Return goes directly back to rebase recovery so `git rebase --continue` remains the only commit-advancing action. Skip and Abort are cancel-first and bound to the exact displayed state. The parent TUI again restores the terminal before each Git control command. Exit status plus current marker and exact `REBASE_HEAD` queries classify completion or a recoverable stop; successful completion requires Git's rebase markers to be gone.
...
The spell checker resolves GNU Aspell only from an absolute PATH entry, fingerprints the executable before each shell-free invocation, and requires the 0.60 or newer pipe protocol. Input is limited to 256 Ki characters; stdout is limited to 4 MiB; stderr is limited to 64 KiB; one result contains at most 4,096 issues and 16 suggestions per issue. `Signed-off-by` and equivalent `*-by` identity trailers are not checked. `gui.spellingdictionary` selects the literal master dictionary, while an empty value leaves dictionary selection to Aspell. Child-tree cancellation, strict UTF-8 decoding, validated Unicode offsets, and control-safe status text apply on every run. Missing executables, invalid dictionaries, non-zero exits, malformed protocol responses, and unsupported versions turn spelling off with an explanation and an explicit retry action; they never disable Commit or discard editor text. SSH-key creation defaults to Ed25519, exposes deliberate alternatives, never overwrites an existing key without confirmation, and treats private-key material as secret.
...
Every rendered frame must completely replace the cells it owns. Full-screen sessions use cell-positioned updates on every platform and disable terminal automatic wrapping before the first frame, so drawing the final column cannot wrap into the next row or shift later writes. Every clean Windows history frame disables automatic wrapping again before repainting. The output queue holds at most one pending item and applies backpressure instead of letting incremental frames accumulate behind a slower terminal. Long-lived editor state replaces its document contents in place so the editor invalidates every changed cell, including blank tails that erase longer text and colored diff spans from the previous preview. Selecting a different history entry starts its preview at the first row and first column; refreshing the same selected commit retains its cursor and viewport. Rapid history-list movement updates focus immediately but waits for the settled preview before performing an expensive clean repaint. On Windows, finishing the delayed preview load or moving its preview viewport by keyboard, wheel, click, or scrollbar drag schedules the clean repaint on the application render thread. GitSail ends any synchronized update before clearing the Windows console screen buffer through the platform console API, then replaces every row using an erase-line command, literal blank cells through the penultimate column, and an explicit final-column erase. Only after the physical buffer is blank does it start and emit the complete synchronized replacement frame. This prevents a terminal from completing a synchronized update against its pre-clear state and restoring untouched cells from the older preview. The redundant row replacement does not depend on erase-display or erase-character support for the main body of each line, and it never streams blanks through the final column, so it cannot create a deferred wrap that shifts later output. If a history update arrives while a frame is already being built, GitSail requests another frame until the newest selection and preview have been built together. Real history and diff tests scroll vertically and horizontally in both directions, change entries during frame construction, replace a long preview with a shorter preview, and verify that the new preview starts at the origin with no text, styling, or shifted right edge from the previous viewport.

Every full-screen exit follows one ordered output path for Ctrl+Q, buttons, cancellation, and failures. After the final frame, GitSail queues a harmless barrier that resets text attributes, disables bracketed paste and mouse reporting, shows the cursor, leaves the alternate screen, and restores terminal automatic wrapping. Shutdown waits until the terminal presentation confirms that barrier was written before disposing the output pump, so an older queued frame cannot repaint GitSail onto the restored shell. A delayed-output terminal test proves that exit returns only after the main screen is restored and contains no application remnants.
...
GitSail UI behavior is tested through the full public headless terminal stack with fixed dimensions, semantic/cell/color assertions, and `WaitUntil` after every state-changing input and immediately before capture. Dedicated comparison tests run at 80×24 and 120×30, select changed files with the mouse, toggle side-by-side and unified layouts through visible controls, require the complete header version and Quit affordances, verify every visible cell of an added line retains the expected foreground and background, require wheel movement to produce identical aligned-editor offsets, prove typing cannot change the read-only document, and move a distant hunk into view. Tests never use `Task.Delay` as synchronization; they wait on observable state or `TaskCompletionSource`. The MTP test applications run on CoreCLR for diagnostics and coverage. `GitSail.AotTests` remains an MTP executable test project that launches and verifies the actual stripped Native AOT GitSail payload; its name does not mean the test runner itself is AOT-published.
...
| `rebase-merge/git-rebase-todo` | bounded read and atomic write only while invoked as the sequence editor; existence read during recovery | exact interactive-rebase plan supplied and owned by Git |
| `rebase-merge/interactive` | bounded existence read only | distinguish an active interactive rebase from a stale `REBASE_HEAD` |
| `rebase-apply/rebasing` | bounded existence read only | detect a recoverable apply-backend rebase |

No other Git-directory path is opened directly. Merge, remaining rebase state, cherry-pick, revert, rerere, reflog, ref, object, `FETCH_HEAD`, hook, common-directory, and worktree state is otherwise queried or changed through Git commands. Tests fail any direct repository-state access not represented in this table.
